...
function generateOperateButtons(buttons, ik) {
  return Object.keys(buttons).map(key => {
    switch (key) {
      ...
      case 'Reset':
        return React.createElement(Reset, {
          options: buttons[key],
          ik: ik
        });
...
function generateValidate(fields, values) {
  ...
  for (let key in fields) {
    ...
    for (let i = 0; i < rules.length; i++) {
      const rule = rules[i]; // 只要有一套规则不过，跳出循环
  ...
  return React.createElement(Formik, {
    ...
    render: ik => {
      ...
      return React.createElement("form", {
        onSubmit: ik.handleSubmit,
        onReset: ik.handleReset
      }, generateFormControls(fields, ik), generateOperateButtons(buttons, ik));
    }
  });
}
;